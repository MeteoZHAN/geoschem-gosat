...
% 提取各站点一天多个值提取为一天一个值
[ae_station] = unique_stations(ae2); % 获取站点某日柱浓度数据
[br_station] = unique_stations(br2); % 获取站点某日柱浓度数据
[db_station] = unique_stations(db2);
[iz_station] = unique_stations(iz2);
[Lamont_station] = unique_stations(Lamont2);
[tk_station] = unique_stations(tk2);
[wg_station] = unique_stations(wg2);
...
for i = 1 : 26   % 2月26个文件
    ...
    lev_sat = ncread(['sat2\',files_sat(i).name],'pressure_levels'); % sat气压层
