...
for i = 1 : 31   % 1月31个文件
    ...
    xch4_sat = ncread(['sat\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
...
for i = 1 : 26   % 1月31个文件
    ...
    xch4_sat = ncread(['sat2\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
...
for i = 1 : 31   % 1月31个文件
    ...
    xch4_sat = ncread(['sat3\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
    %     scatter(lon_sat,lat_sat)
    [m_lon,m_lat] = meshgrid(lon_model,lat_model);
    % 从高度插值后的模式数据选取出卫星观测位置的数据
    for n = 1 : length(lon_sat)
...
for i = 1 : 30   % 1月31个文件
    ...
    xch4_sat = ncread(['sat4\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
    %     scatter(lon_sat,lat_sat)
    [m_lon,m_lat] = meshgrid(lon_model,lat_model);
    % 从高度插值后的模式数据选取出卫星观测位置的数据
    for n = 1 : length(lon_sat)
...
for i = 1 : 25   % 1月31个文件
    ...
    xch4_sat = ncread(['sat5\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
    %     scatter(lon_sat,lat_sat)
    [m_lon,m_lat] = meshgrid(lon_model,lat_model);
    % 从高度插值后的模式数据选取出卫星观测位置的数据
    for n = 1 : length(lon_sat)
...
for i = 1 : 30   % 1月31个文件
    ...
    xch4_sat = ncread(['sat6\',files_sat(i).name],'xch4_uncertainty');  % 单位：ppb
    %     scatter(lon_sat,lat_sat)
    [m_lon,m_lat] = meshgrid(lon_model,lat_model);
    % 从高度插值后的模式数据选取出卫星观测位置的数据
    for n = 1 : length(lon_sat)
...
plot(1:6,usa_ave_sat)
hold on
plot(1:6,cn_ave_sat)
plot(1:6,aus_ave_sat)
plot(1:6,eu_ave_sat)
xlabel('月份')
ylabel('ppb')
legend('usa','cn','aus','eu')
xlim([0.5,6.5])
ylim([11.5,17.5])
disp('计算完毕！！！')